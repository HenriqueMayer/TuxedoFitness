...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        exercise = (
            ExerciseTemplate.objects.filter(
        ...
        context["presentation"] = DashboardPresenter(
            getattr(self.request.user, "fitness_preferences", None), self.request.GET
        ).present_overview(context["overview"], prepared=prepared)
        if compare:
            current = context["overview"]["period"]
            previous_end = current.start - timedelta(days=1)
            previous = service.period(
                previous_end - timedelta(days=current.days - 1), previous_end
            )
            context["comparison"] = service.compare_periods(
                lambda selected: service.workout_activity(selected)["workouts"],
                current,
                previous,
            )
        context["freshness"] = IntegrationState.objects.filter(
            hevy_account=account
        ).first()
...
class ReportsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        selected_panels = (
            preference.panels if preference else [key for key, _ in PANELS]
        )
        panel = self.request.GET.get(
            "panel", selected_panels[0] if selected_panels else "frequency"
        )
        ...
        data = (
            self.request.GET
            if self.request.GET
            else (preference.filters if preference else {})
        )
        ...
        if account and form.is_valid():
            context.update(
                build_reports(account, form.cleaned_data, panel, query=self.request.GET)
            )
        return context
    ...
    def post(self, request, **kwargs):
        ...
        preference, _ = DashboardPreference.objects.get_or_create(
            user=request.user, defaults={"panels": [key for key, _ in PANELS]}
        )
        ...
        if request.POST.get("action") == "save_filters":
            form = ReportForm(
                ...
                panel=request.POST.get("panel", "frequency"),
            )
            if form.is_valid():
                preference.filters = {
                    key: str(getattr(value, "pk", value)) if value is not None else ""
            ...
            preference.panels = [key for key in order if key in selected] + [
