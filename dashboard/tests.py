...
class ProtectedShellTests(TestCase):
    ...
    def test_shell_assets_are_local_and_csp_blocks_inline_code(self):
        ...
        self.assertIn('/static/css/app.css?v=1', html)
    ...
    def test_dashboard_renders_confirmed_local_metrics(self):
        ...
        self.assertContains(response, 'Treinos')
    ...
    def test_dashboard_accepts_reproducible_date_query(self):
        ...
        response = self.client.get(
            ...
            {'inicio': '2026-01-01', 'fim': '2026-01-03'},
        )

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, '01/01/2026 a 03/01/2026')
    ...
    def test_dashboard_comparison_is_opt_in(self):
        ...
        response = self.client.get(reverse('dashboard:index'), {'comparar': 'on'})
    ...
    def test_dashboard_exposes_server_svg_and_equivalent_table(self):
        ...
        self.assertContains(response, '<title id="activity-chart-svg-title">')
        self.assertContains(response, '<desc id="activity-chart-svg-desc">')
        self.assertNotContains(response, 'plotly', html=False)
        self.assertNotContains(response, 'application/json')
        self.assertContains(response, 'tabela equivalente')
        self.assertContains(response, 'aria-labelledby="activity-chart-svg-title activity-chart-svg-desc"')
    ...
    def test_dashboard_chart_is_safe_and_no_data_is_explicit(self):
        ...
        response = self.client.get(reverse('dashboard:index'), {
            'inicio': '2030-01-01', 'fim': '2030-01-28',
        })

        self.assertNotContains(response, 'activity-chart-svg-title')
        self.assertContains(response, 'Nenhum treino confirmado no período selecionado.')
    ...
    def test_dashboard_hx_request_returns_only_exact_results_target(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.content.lstrip().startswith(b'<div id="overview-results">'), response.content[:120])
