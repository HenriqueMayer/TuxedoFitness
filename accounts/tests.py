...
class SignupTests(TestCase):
    ...
    def test_registration_remains_available_for_another_user(self):
        ...
        self.assertRedirects(response, reverse('dashboard:index'))
        self.assertTrue(get_user_model().objects.filter(username='second-user').exists())
...
class OwnerPreferenceTests(TestCase):
    ...
    @mock.patch('accounts.views.create_verified_backup', return_value=Path('/tmp/synthetic-backup.sqlite3'))
    ...
    @mock.patch('accounts.views.create_verified_backup', side_effect=BackupError('Backup unavailable.'))
...
class BackupServiceTests(TransactionTestCase):
    def test_backup_is_created_outside_checkout_and_passes_integrity(self):
        with tempfile.TemporaryDirectory() as backup_dir, mock.patch.dict(
            os.environ, {'TUXEDO_FITNESS_BACKUP_DIR': backup_dir}
        ):
    ...
    def test_isolated_restore_copy_passes_database_checks(self):
        with tempfile.TemporaryDirectory() as backup_dir, tempfile.TemporaryDirectory() as restore_dir, mock.patch.dict(
            os.environ, {'TUXEDO_FITNESS_BACKUP_DIR': backup_dir}
        ):
            backup = create_verified_backup()
