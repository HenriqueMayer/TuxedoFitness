...
class HealthTests(SimpleTestCase):
    ...
    def test_hsts_is_sent_only_on_https(self):
        self.assertNotIn(
            "Strict-Transport-Security", self.client.get(reverse("health")).headers
        )
        secure = self.client.get(reverse("health"), secure=True)
        self.assertEqual(
            secure.headers["Strict-Transport-Security"], "max-age=31536000"
        )
...
class SecretScanTests(SimpleTestCase):
    ...
    def test_private_runtime_paths_are_rejected(self):
        ...
        self.assertTrue(
            prohibited_private_path(PurePosixPath("data/Hevy/routines.json"))
        )
        self.assertTrue(
            prohibited_private_path(PurePosixPath("var/private/app.sqlite3"))
        )
...
class FrontendVersionTests(SimpleTestCase):
    def test_asset_url_changes_when_contents_change_without_an_app_version_bump(self):
        from core.assets import versioned_static

        with tempfile.TemporaryDirectory() as directory:
            css = Path(directory) / "app.css"
            css.write_text(".new { color: red; }")
            with mock.patch("core.assets.finders.find", return_value=str(css)):
                first = versioned_static("css/app.css")
                css.write_text(".new { color: blue; }")
                self.assertNotEqual(first, versioned_static("css/app.css"))
                self.assertNotIn("v=020", first)

    def test_old_htmx_tabs_reload_the_requested_get_but_never_replay_posts(self):
        from django.http import HttpResponse
        from django.test import RequestFactory

        from core.assets import frontend_version
        from core.middleware import FrontendVersionMiddleware

        view = mock.Mock(return_value=HttpResponse("current page"))
        middleware = FrontendVersionMiddleware(view)
        factory = RequestFactory()
        for revision in (None, "old-version"):
            headers = {"HTTP_HX_REQUEST": "true"}
            if revision:
                headers["HTTP_X_FRONTEND_VERSION"] = revision
            response = middleware(
                factory.get("/dashboard/?start=2026-09-01", **headers)
            )
            self.assertEqual(response["HX-Redirect"], "/dashboard/?start=2026-09-01")
        ...
        view.reset_mock()
        middleware(factory.post("/planning/", HTTP_HX_REQUEST="true"))
        view.assert_called_once()
