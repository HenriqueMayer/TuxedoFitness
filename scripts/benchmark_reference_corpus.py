...
def run(database_path):
    ...
    Workout.all_objects.bulk_create([
        Workout(
            ...
            end_time=now - timedelta(days=index % 3650, minutes=index % 1440) + timedelta(hours=1),
            **synchronized,
        )
        for index in range(10_000)
    ], batch_size=1_000)
    workout_ids = Workout.all_objects.filter(hevy_account=account).order_by('pk').values_list('pk', flat=True)
